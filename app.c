...
unsigned char requestDirection = 1;  // 0= down , 1= up
...

{
    ...
    if (!button_read(BUTTON_G_PORT_DR, BUTTON_G_PIN)) //G
    {
        selectedFloor = 0;
        getDirection();
    }
    if (!button_read(BUTTON_1_PORT_DR, BUTTON_1_PIN)) // 1
    {
        selectedFloor = 1;
        getDirection();
    }
    if (!button_read(BUTTON_2_PORT_DR, BUTTON_2_PIN)) // 2
    {
        selectedFloor = 2;
        getDirection();
    }
    if (!button_read(BUTTON_3_PORT_DR, BUTTON_3_PIN)) // 3
    {
        selectedFloor = 3;
        getDirection();
    }
    if (!button_read(BUTTON_4_PORT_DR, BUTTON_4_PIN)) // 4
    {
        selectedFloor = 4;
        getDirection();
    }
